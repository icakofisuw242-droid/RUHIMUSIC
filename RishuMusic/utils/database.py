...
from RishuMusic import userbot
...
async def get_userss(chat_id: int) -> Dict[str, int]:
    ids = await usersdb.find_one({"chat_id": chat_id})
    if not ids:
        return {}
    return ids["vidid"]
...
async def update_user_top(chat_id: int, name: str, vidid: dict):
    ids = await get_userss(chat_id)
    ids[name] = vidid
    await usersdb.update_one({"chat_id": chat_id}, {"$set": {"vidid": ids}}, upsert=True)


async def get_topp_users() -> dict:
    results = {}
    async for chat in usersdb.find({"chat_id": {"$gt": 0}}):
